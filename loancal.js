        ...
        window.onload = function() {
            ...
            loanForm.addEventListener('submit', function(e) {
                ...
                var monthly = 0,
                    ...
                    totalInterest = 0,
                    simpleInterest = 0;
                simpleInterest = (parseFloat(principalAmount) * (parseFloat(interest) / 100));
                // pmt = parseFloat(simpleInterest) / (1 - Math.pow(1 + ((parseFloat(interest) / 100) / 12), -12 * parseFloat(PayableYears)));
                pmt = (parseFloat(principalAmount) / PayableYears) + simpleInterest;
                total = (parseFloat(principalAmount) + (parseFloat(simpleInterest) * PayableYears));
                // total = parseFloat(pmt) * Math.floor(parseFloat(PayableYears) * 12);
                totalInterest = parseFloat(simpleInterest) * parseFloat(PayableYears);
                setTimeout(() => {
                    ...
                    document.getElementById('loan-terms').textContent = parseFloat(PayableYears).toLocaleString("en-US", { style: "decimal", maximumFractionDigits: 2 }) + (PayableYears > 1 ? " Mth(s)." : "")
